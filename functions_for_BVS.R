...
TGS<-function(p,gamma_start=NULL,
  ...
  ## initialize vector to be output
  est_inclusion_probs_mat <- matrix(NA, nrow = T, ncol = p)
  est_inclusion_probs<-rep(0,p)
  ...
  for (t in 1:(T+burn_in)){
    ...
    if(t>burn_in){
      ...
      est_inclusion_probs_mat[t- burn_in,] <- est_inclusion_probs
  ...
  return(list(est_inclusion_probs=est_inclusion_probs,
              est_inclusion_probs_mat=est_inclusion_probs_mat,
              indices_sequence=indices_sequence,
  ...
  for (t in 1:(T+burn_in)){
   ...
   if(length(which(output_fc$fc==0))>0){
      ...
      j<-which(cumsum(flip_rates)>(runif(1)*sum(flip_rates)))[1]
        #which(cumsum(flip_rates + hyper_par$prior_p_incl)>(runif(1)*(sum(flip_rates) + hyper_par$prior_p_incl * p)))[1]
    }
    gamma[j]<-1-gamma[j]
    ## compute full conditionals and update Rao-Blackwelied estimator
    output_fc<-full_cond(gamma=gamma,hyper_par=hyper_par,stored=output_fc$stored)
...
single_full_cond<-function(j,gamma,hyper_par=NULL,stored=NULL){
  ...
  if(p_gamma==0){
    S_old<-yty
    S_new<-yty-ratio_c*ytX[j]^2/XtX[j,j]
    post_ratio<-ratio_h*(S_old/S_new)^(n/2)/(sqrt(1+c))
    ...
    if(gamma[j]==0){
      ...
      S_new<-yty-ratio_c*(ytXFXty+d*(ytXFXtxj-ytX[j])^2)
      post_ratio<-ratio_h*(S_old/S_new)^(n/2)/(sqrt(1+c))
      fc_j<-1/(1+post_ratio)# fc is the full cond prob of the current value of gamma[j]
    }
    if(gamma[j]==1){
      if(p_gamma==1){
        ...
        S_new<-yty-ratio_c*ytX_gamma_new%*%F_new%*%t(ytX_gamma_new)
      }
...
full_cond<-function(gamma,hyper_par=NULL,stored=NULL){
  ...
  if(p_gamma==0){
    ...
    S_new_vec<-yty-ratio_c*(ytXFXty+d_vec*(ytXFXtxj_vec-ytX)^2)
    post_ratio_vec<-ratio_h*(S_old/S_new_vec)^(n/2)/(sqrt(1+c))
